...
def get_duration_and_fps(input_media_path):
    """
    Extract duration and optional FPS/resolution from a media file using FFprobe.
    Simplified for audio-only (e.g., WebM, MP3) and video files.
    """
    duration = None
    fps = None
    width = None
    ...
    try:
        ...
        # Fallback to stream duration (audio or video)
        if duration is None:
            for stream in data.get('streams', []):
                if 'duration' in stream:
                    duration = float(stream['duration'])
                    break
                # Handle tags.DURATION for WebM/Opus
                if stream.get('tags', {}).get('DURATION'):
        ...
        print(f"\033[1;33mWarning: FFprobe failed to parse metadata for {input_media_path}: {e}\033[0m")

    # Fallback: Direct duration probe
    if duration is None:
        try:
            result = subprocess.run(
...
if __name__ == '__main__':
    print(f"Eventogrammer, version 5.0.6,  with dynamic window rendering. Notes: a file of duration of 30 mins requires 6GB RAM to process, with the time ratio: 1 original second : 10 seconds to process. An adaptation of: https://github.com/qiuqiangkong/audioset_tagging_cnn")
    print(f"")
