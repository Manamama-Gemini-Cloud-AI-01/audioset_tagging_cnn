...
def sound_event_detection(args):
    ...
    if args.dynamic_eventogram:
        ...
        def make_frame(t):
            ...
            axs[1].yaxis.set_ticks(np.arange(0, top_k))
            # Use annotations for fading labels
            for i, label in enumerate(window_labels):
                alpha = 1.0 - min(1.0, (t % 1) * 2) if i >= len(window_labels) // 2 else 1.0  # Fade lower half
                axs[1].annotate(label, xy=(-0.05, i), xycoords=('axes fraction', 'data'), fontsize=14, va='center', alpha=alpha)
            axs[1].yaxis.grid(color='k', linestyle='solid', linewidth=0.3, alpha=0.3)
            axs[1].set_xlabel('Seconds', fontsize=14)
            axs[1].xaxis.set_ticks_position('bottom')

            # Adjust x-axis ticks for window
            window_seconds = window_output.shape[0] / frames_per_second
            tick_interval_window = max(1, int(window_seconds / 5))
            x_ticks_window = np.arange(0, window_output.shape[0] + 1, frames_per_second * tick_interval_window)
            x_labels_window = np.arange(int(start_frame / frames_per_second), int(end_frame / frames_per_second) + 1, tick_interval_window)
            axs[1].xaxis.set_ticks(x_ticks_window)
            axs[1].xaxis.set_ticklabels(x_labels_window[:len(x_ticks_window)], rotation=45, ha='right', fontsize=10)
            axs[1].set_xlim(0, window_output.shape[0])

            # Add marker
            marker_x = (t - start_frame / frames_per_second) * frames_per_second
            if 0 <= marker_x <= window_output.shape[0]:
                axs[1].axvline(x=marker_x, color='red', linewidth=2, alpha=0.8)
        ...
        eventogram_clip = CompositeVideoClip([ImageClip(make_frame(t), duration=1/fps) for t in frame_times])
        ...
        marker = ColorClip(size=(2, static_eventogram_clip.h), color=(255, 0, 0)).set_duration(duration)
        marker = marker.set_position(marker_position)
