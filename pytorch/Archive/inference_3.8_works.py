    ...
    try:
        ...
        for track in media_info.tracks:
            if track.track_type == "Video":
                duration = float(track.duration) / 1000.0 if track.duration else None
                fps = float(track.frame_rate) if track.frame_rate else None
                width = track.width
                height = track.height
                break
            elif track.track_type == "Audio" and duration is None:
                duration = float(track.duration) / 1000.0 if track.duration else None
        ...
        if fps:
            print(f"🗃️  Input video FPS: \033[1;34m{fps:.3f}\033[0m")
        if width and height:
        ...
        print(f"\033[1;31mFailed to parse MediaInfo: {e}\033[0m")
...
def sound_event_detection(args):
    ...
    gs = fig.add_gridspec(2, 1, height_ratios=[3, 1], left=0.0, right=1.0, top=0.95, bottom=0.08, hspace=0.05)
    ...
    axs[1].xaxis.set_ticklabels(x_labels[:len(x_ticks)], rotation=45, ha='right')
    ...
    axs[1].set_yticklabels(top_labels, fontsize=8, rotation=0, ha='right', va='center')  # rotate if needed
    ...
    axs[1].yaxis.set_ticklabels(top_labels, fontsize=8)
    axs[1].yaxis.grid(color='k', linestyle='solid', linewidth=0.3, alpha=0.3)
    axs[1].set_xlabel('Seconds')
    axs[1].xaxis.set_ticks_position('bottom')
    ...
    if is_video:
        ...
        original_video_clip = VideoFileClip(audio_path, fps_source='fps')

        eventogram_with_audio_clip = eventogram_with_audio_clip.with_fps(original_video_clip.fps)

        
        # Resize overlay: we preserve aspect ratio and set overlay height as fraction of video height
    ...
    print(f"🮲  Output video FPS: \033[1;34m{fps:.3f}\033[0m")
