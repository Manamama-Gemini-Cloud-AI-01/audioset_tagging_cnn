...
def sound_event_detection(args):
    ...
    if is_video:
        ...
        if data['streams']:
            ...
            if r_frame_rate != avg_frame_rate:
                ...
                subprocess.run([
                    'ffmpeg', '-i', audio_path, '-r', str(video_fps), '-vsync', '1', '-c:a', 'copy', temp_video_path, '-y'
                ], check=True)
                video_input_path = temp_video_path
                # Update fps, width, height if needed, but since no change, keep
    ...
    # --- CSV and SRT output (same as before) ---
    threshold = 0.5
    all_events = []
