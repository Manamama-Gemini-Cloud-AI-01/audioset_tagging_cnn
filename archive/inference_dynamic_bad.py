...
from moviepy import ImageClip, CompositeVideoClip, AudioFileClip, ColorClip, concatenate_videoclips
import json
...
def sound_event_detection(args):
    print("DEBUG: Starting sound_event_detection")
    sample_rate = args.sample_rate
    window_size = args.window_size
    hop_size = args.hop_size
    mel_bins = args.mel_bins
    ...
    print("DEBUG: Model loaded. Getting duration and FPS...")
    ...
    print("DEBUG: Entering dynamic/static eventogram logic...")
    ...
    if not args.dynamic_eventogram:
        ...
        final_clip.write_videofile(output_video_path, codec="libx264", fps=video_fps if video_fps else 24)
        print("DEBUG: Final video written.")
    ...
    if is_video and temp_video_path and os.path.exists(temp_video_path):
        try:
            ...
            print(f"\033[1;33mWarning: Failed to delete temporary CFR video {temp_video_path}: {e}\033[0m")
    print("DEBUG: Finished sound_event_detection")

def detect_scenes(framewise_output, frames_per_second, scene_change_threshold=5):
    """
    Analyzes framewise_output to detect scenes based on changes in top events.
    
    Args:
        framewise_output (np.ndarray): The output from the model.
        frames_per_second (int): Number of frames per second.
        scene_change_threshold (int): Number of top events that need to change to trigger a new scene.
        
    Returns:
        list of dicts: A list of scenes with start and end times and top labels.
    """
    ...
    # Analyze in chunks of 1 second
    for frame_index in range(0, num_frames, frames_per_second):
        
        chunk = framewise_output[frame_index:frame_index + frames_per_second]
        if chunk.shape[0] == 0:
            continue
            
        # Get top indices for the current chunk
        max_probs_in_chunk = np.max(chunk, axis=0)
        current_top_indices = set(np.argsort(max_probs_in_chunk)[-top_k:])
        
        if last_top_indices is None:
            last_top_indices = current_top_indices
            continue
            
        # Check for scene change
        if len(last_top_indices.symmetric_difference(current_top_indices)) >= scene_change_threshold:
            
            scene_end_frame = frame_index
            
            # Determine top labels for the completed scene
            scene_chunk = framewise_output[scene_start_frame:scene_end_frame]
            scene_max_probs = np.max(scene_chunk, axis=0)
            scene_top_indices = np.argsort(scene_max_probs)[-top_k:]
            
            scenes.append({
                'start_time': scene_start_frame / frames_per_second,
                'end_time': scene_end_frame / frames_per_second,
                'top_indices': scene_top_indices
            })
    ...
    # Add the last scene
    scene_chunk = framewise_output[scene_start_frame:]
    scene_max_probs = np.max(scene_chunk, axis=0)
    scene_top_indices = np.argsort(scene_max_probs)[-top_k:]
    
    scenes.append({
        'start_time': scene_start_frame / frames_per_second,
        'end_time': num_frames / frames_per_second,
        'top_indices': scene_top_indices
    })
...
if __name__ == '__main__':
    ...
    parser.add_argument('--dynamic_eventogram', action='store_true', default=False, help='Enable dynamic eventogram generation')
